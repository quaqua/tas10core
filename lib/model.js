...
var Property = require( __dirname + '/props/property' )
  , CreioString = require( __dirname + '/props/creio_string' )
  , CreioInt = require( __dirname + '/props/creio_int' )
  , CreioArray = require( __dirname + '/props/creio_array' )
  , CreioObject = require( __dirname + '/props/creio_object' )
  , CreioDateTime = require( __dirname + '/props/creio_datetime' )
  , CreioBoolean = require( __dirname + '/props/creio_boolean' );
...
Model._baseSchema = function(){
  ...
  return {
    className: { type: CreioString },
    name: { type: CreioString, index: true, default: null, required: true },
    history: { type: CreioArray, default: [] },
    acl: { type: CreioObject, default: {} },
    position: { type: CreioInt, default: 999 },
    labels: { type: CreioArray, default: [] },
    starred: {type: CreioBoolean }
  }
...
//                    {type: CreioDateTime, default: new Date() }})
//
// also basic validations can be passed.
// ## Options
// [type] - The type this value should always be set with.
// Available types are: CreioString, CreioInt, CreioArray, 
// CreioObject, CreioDateTime, ...
...
Model.schema = function(){
  for( var i in arguments ){
    ...
    if( typeof(props) === 'object' )
      ...
      this._schema[ props ] = { type: CreioString };
  }
  return this._schema;
}
...
Model.prototype._initProps = function(){
  ...
  this._props.className = CreioString.new(this.constructor.name);
...
Model.prototype._setupProps = function( properties ){
  for( var i in properties ){
    ...
    /* this would set up any value coming into the constructor:
      ...
      this._props[i] = CreioString.new( properties[i] );
    */
      this[i] = this._props[i].getValue();
    } else
      this[i] = properties[i];
