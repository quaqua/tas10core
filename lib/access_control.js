...
function share( user, privileges ){
	...
	this.constructor.logger.info( 'SHARED name: ' + this.name + ' id: '+this._id+' with user: '+user.name + ' (' + privileges + ') user: ' + (this.holder && this.holder.name) );
...
function unshare( user ){
	...
	this.constructor.logger.info( 'UNSHARED name: ' + this.name + ' id: '+this._id+' user: '+user.name + ' no longer has access. user: ' + (this.holder && this.holder.name) );
}
