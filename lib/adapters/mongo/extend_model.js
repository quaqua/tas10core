...
module.exports = function( Model, db, logger ){
			...
			function finishInsert(){
				self.constructor.collection().insert( self.normalizeProps(), {safe: true}, function(err, newDocumentProps){
					...
					if( err )
						...
						logger.info( 'INSERTED name: ' + self.name + ' id: '+newDocumentProps[0]._id+' user: ' + (self.holder && self.holder.name) );
						...
						throw new Error("tas10core specification transgression. Please only insert one document object at time!");
				});
			};
	...
	Model.prototype.addLabel = function( doc ){
		...
		logger.info( 'LABELED name: ' + this.name + ' id: '+this._id+' label: ' + doc.name + ' label_id: ' + doc._id + ' user: ' + (this.holder && this.holder.name));

		this.label_ids.indexOf( doc._id ) < 0 && this.label_ids.push( doc._id );
	  	for( var i in doc.acl )
	    	if( ! this.acl[i] ){
	      		this.acl[i] = doc.acl[i];
   				logger.info( 'LABELED (2) ACL EXTENDED (from previous labeleing) user: ' + i + ' privileges: ' + doc.acl[i] );
	      	}
	}
	...
	Model.prototype.removeLabel = function( doc ){
		if( doc._id && this.label_ids.indexOf( doc._id ) < 0 ){
			...
			logger.info( 'REMOVED LABEL ' + doc.name + ' label_id: ' + doc._id + ' from ' + this.name + ' id: ' + this._id, + ' user: ' + (this.holder && this.holder.name) );
			...
			self.constructor.collection().update( {_id: self._id}, self.normalizeProps(), {safe: true}, 
				function( err, numChangedRecords ){
					...
					if( err === null && numChangedRecords === 1 ){
						...
						logger.info( 'UPDATED name: ' + self.name + ' id: '+self._id+' user: ' + (self.holder && self.holder.name) );
			...
			self.constructor.collection().remove( {_id: self._id}, {safe: true}, function( err, numDestroyed ){
				...
				if( err )
					...
					self.constructor.collection().update({'label_ids': self._id}, {$pull: {'label_ids': self._id}}, {safe: true},
						...
						logger.info( 'DELETED name: ' + self.name + ' id: '+self._id+' user: ' + (self.holder && self.holder.name));
