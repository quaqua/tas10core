...
module.exports = function( Model, db, logger ){
	...
	Model.prototype.save = function save( callback ){		
		...
		//this.runHooks( 'before', 'save', function continueSaveBeforeSaveHooks(){
	...
	Model.prototype._insert = function _insert( callback ){
		...
		// self.runHooks( 'before', 'create', function continueInsertBeforeCreateHooks(){
			...
			function finishInsert(){
				self.constructor.collection().insert( self.normalizeProps(), {safe: true}, function(err, newDocumentProps){
					...
					if( err )
						...
						logger.info( 'INSERTED \'' + newDocumentProps.name + '\'' );

						doc = self.constructor.new(self.holder, newDocumentProps[0]);

						//self.runHooks( 'after', 'create', function continueInsertAfterCreateHooks(){

							//self.runHooks( 'after', 'save', function continueInsertAfterSaveHooks(){
						...
						throw "tastenbox-core specification transgression. Please only insert one document object at time!";
				});
			};
...
*/
	...
	Model.prototype._update = function _update( props, callback ){
		callback(null, this);
	}
